...
class GraphforPathPlan:
    ...
    def create_graph(self):
        ...
        for i in range(0, self.height, robot_pix):
            ...
            for j in range(0, self.width, robot_pix):
                ...
                idx = np.where(block > COSTMAP_OCCUPANCY_VAL)  #### MJ
                if block_id == self.robot.robot_id:
                    # if 0 <= avg <= 20:  #### MJ
                    if 0 <= avg <= COSTMAP_OCCUPANCY_VAL:
    ...
    def get_node(self, pose):
        ...
        if 0 <= i < self.resized_height and 0 <= j < self.resized_width:
            ...
            random_node = random.choice(random.choice(self.nodes))
            print("random select {}".format(random_node))
            return random_node
            ...
            # if j < 0:
            #     j = 0
            # print("resized {}".format((self.resized_height, self.resized_width)))
            # print("Not Found i:%d j:%d" % (i, j))
            # return self.nodes[i, j]
    ...
    def pose_to_index(self, pose):
        ...
        y = pose.position.y
        print("Y init: ", y)
        print("origin {}".format((self.origin.position.x, self.origin.position.y)))
        x = x - self.origin.position.x
        y = y - self.origin.position.y  ## MJ
        print("Y update: ", y)
...
class PathPlan:
    ...
    def __init__(self, occ_grid, map_info, tesselation_image, robot):
        self.graph = GraphforPathPlan(occ_grid, map_info, tesselation_image, robot)
        self.robot = robot
        ...
        print("Start node is retreived {}".format(self.start_node))
    ...
    def boustrophedon_motion(self, node):
        ...
        max_i = self.graph.resized_height  # i: y-direction (height, row)
        max_j = self.graph.resized_width  # j: x-direction (width, col)
    ...
    def backtrackin_f(self, node):
        ...
        i, j = node.get_index()
        # neighbors = [(i, j + 1), (i - 1, j), (i, j - 1), (i + 1, j)]  ## MJ
        # s = [None] * 4
    ...
    def calc_backtrack(self, M):
        ...
        if not dist:
            print("Not dist Scp: {}".format(dist))
            return None
    ...
    def plan(self):
        ...
        print("Path planning is started! {}".format(self.start_node))
        iteration = 0
        M_L = []
        while not rospy.is_shutdown():
            iteration += 1
